...
cleanup() {
    ...
    sudo losetup -l | grep -v ^NAME | while read dev b c d e file f g ; do
...
unit b
mklabel gpt
mkpart "EFI System Partition" fat32 1048576 $(sub "${imgsize}" 17280)
set 1 boot on
EOF

part=$(sudo kpartx -a -v -p p eml.img | while read a b c d ; do echo "${c}" ; break ; done)
